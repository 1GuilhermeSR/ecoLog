...
export default function EmissaoCombustivel() {
    ...
    const emissoesFiltradas = useMemo(() => {
        ...
        return emissoes.filter((emissao) => {
            ...
            return (
                matchData || matchKmPerc || matchMediaKm || matchCo2 || matchComb || matchDataParcial || matchMesNome
            );
        });
    }, [emissoes, termoBusca]);
    ...
    function editar(record: EmissaoCombustivelDTO) {
        setCurrentItem(record);
        setIsModalOpen(true);
    }

    async function excluir(record: EmissaoCombustivelDTO) {
        if (!record.id) return;
        setLoading(true);
        try {
